...
def dXdT(t, X):
    ...
    # Changed this section to now allow alteration
    #  of the key conductances and permeabilies in this
    #  parameter value section of the code
    g_Na_max = 400                            # Max Na channel cond (mS/cm^2)
    P_CaLmax = 0.00054 * 2.5                  # L-type Ca chnl perm (cm/s)
    KmCa = 0.6                                # L-type Ca fCa gt half act (uM)
    g_K1max = 1.2 * 0.60                      # Time-dep K1 max cond (mS/cm^2)
    g_K2max = 1.2 * 0.77                      # Time-dep K max cond (mS/cm^2)
    ...
    # Here we have altered the stimulation protocol to allow us to 
    #  put in a list of two stim times to pace the simulation
    Deltat = 20                               # Stim duration (ms)
    Iapp_Amp = 5.0                             # Applied current amp (uA/cm^2)
    StimTimes = [1430, 4160]
    Stim_Flag = 0
    for i in range(len(StimTimes)):
        if (Stim_Flag == 0):
            if ((t - StimTimes[i]) >= 0 and (t - StimTimes[i]) <= Deltat):
                I_App = Iapp_Amp
                Stim_Flag = 1
            else:
                I_App = 0
    ...
    # Note here we have added back in a bit of a Na leak to try to 
    #  capture the slight depolarization that occurs in diastole
    i_Na = (g_Na + 0.05) * (V - E_Na)         # Na channel current
    ...
    g_K1 = (g_K1max * np.exp(((-1*V) - 90) / 50)) +\
        (0.015 * np.exp((V + 90) / 60))         # K chnl inwrd rct cond (eqn 5)
    g_K2 = g_K2max * n**4                       # K ch outwrd rect cond (eqn 6)
    ...
    i_CaL = P_CaLmax * d * f * f_Ca * (z_Ca**2 * F * VFoRT) *\
...
# Load WiCell data (time and arbitrary units of florescence)
#  This dataset came from InvivoSciences and optical data from 
#  an iPSC-CM derived from a female patient 
Data = np.loadtxt("./WiCell_Data.txt")

# Now normalize the florescence by dividing by the max florescence 
...
tspan = np.arange(0., 6500., 0.1)
results = solve_ivp(dXdT, [0., 6500], [-89, 0.1, 0.1, 0.01, 0.1, 0.1],\
...
# Comparison between fluorescence data and normalized simulation voltages
plt.figure(4)
MaxV = np.max(results.y[0])
MinV = np.min(results.y[0])
plt.plot(Time,NormFlrsnc)
plt.plot(results.t/1000,(results.y[0] - MinV)/(MaxV - MinV))
plt.xlabel('Time (s)')
plt.ylabel('Normalized Fluorescence')