# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 09:41:31 2023
...
data=pd.read_csv('WiCell_Data.txt', delim_whitespace=True, header=None, names=["Time", "Fluorescence"])

#normalized data
norm= (data.Fluorescence-np.min(data.Fluorescence))/np.ptp(data.Fluorescence)
...
def dXdT(t, X):
    ...
    P_CaL = 2.5*0.00054                           # L-type Ca chnl perm (cm/s)
    ...
    #Period = 1458                             # Stim frequency period (ms)
    #Deltat = 10;                            # Stim duration (ms)
    #Iapp_Amp = 35;                            # Applied current amp (uA/cm^2)
    ...
    i_Na = (g_Na +.05) * (V - E_Na)         # Na channel current
    ...
    g_K1 = (1.2*.6 * np.exp(((-1*V) - 90) / 50)) +\
        (0.015 * np.exp((V + 90) / 60))        # K chnl inwrd rct cond (eqn 5)
    g_K2 =1.2*.77 * n**4      #multiple by .77 fix laterh outwrd rect cond (eqn 6)
    ...
    i_CaL =1.2*( (P_CaL) * d * f * f_Ca * (z_Ca**2 * F * VFoRT) *\
...
tspan = np.arange(0,6500,.1)
results = solve_ivp(dXdT, [0., 6500], [-89, 0.1, 0.1, 0.01, 0.1, 0.1],\
        method = 'Radau', atol = 1e-9, max_step = 0.25, t_eval = tspan)
#normalize the results
normmodel=(results.y[0]-np.min(results.y[0]))/np.ptp(results.y[0])  
plt.plot(results.t/1000,normmodel)
maxflur=np.max(data.Fluorescence)
normfl=data.Fluorescence/maxflur
#plt.plot(results.t/1000,normfl)
#plt.plot(data.Time,data.Fluorescence)
#plt.xlabel('Time (s)')
#plt.ylabel('Normalized Potential')
#plt.show()
plt.plot(data.Time,normfl)
plt.xlabel('Time (s)')
